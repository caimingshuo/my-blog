---
layout: doc
---

<style>
.VPDoc h2 {
  font-family: 'Playfair Display', 'Noto Serif SC', serif !important;
  font-weight: 500;
  font-size: 1.5rem;
  margin-top: 48px;
  margin-bottom: 16px;
  padding-bottom: 8px;
  border-bottom: 1px solid rgba(255, 255, 255, 0.1);
}

.VPDoc h2:first-of-type {
  margin-top: 24px;
}
</style>

# Articles

所有文章列表。

## AI & 技术

- [Harness AI Agent：DevOps 智能化的下一代范式](./harness-ai-agent.md) - *Apr 22, 2026* - Harness 平台的 AI Agent 体系，从 Pipeline 自动生成到自主运维的 DevOps 智能化实践

- [Consistency Model 与 Latent Consistency Model：扩散模型的少步生成革命](./lcm-consistency-model.md) - *Apr 23, 2026* - 从单步生成的理论基石到 LCM-LoRA 的即插即用加速，解析扩散模型的少步生成范式

- [SoulX-FlashTalk & FlashHead：消费级 GPU 实时数字人技术](./soulx-flashtalk-flashhead.md) - *Mar 30, 2026* - Soul App AI Lab 开源的两个实时数字人项目，14B 云端高质量与 1.3B 消费级实时方案

- [AutoResearch: 让 AI Agent 自主进行深度学习研究](./autoresearch-karpathy.md) - *Feb 15, 2026* - Andrej Karpathy 的实验性项目，探索让 AI Agent 自主进行深度学习研究

- [Building AI Agents: A Comprehensive Guide](./agent-development-guide.md) - *Feb 28, 2026* - 深入探讨 AI Agent 架构，包括 ReAct、Plan-and-Execute 和 Multi-Agent 模式

## 工具 & 教程

- [如何使用 VitePress 搭建个人博客](./building-blog.md) - *Dec 15, 2025* - 从零开始使用 VitePress 搭建一个简洁高效的静态博客

## 杂记

- [我的桌面装备 2026](./my-desk-setup-2026.md) - *Mar 28, 2026* - 一个研究生的桌面EDC分享：MacBook、4K显示器、机械键盘、降噪耳机等

- [Weekly Reading #1 - 有趣的链接分享](./weekly-reading.md) - *Jan 10, 2026* - 本周阅读的一些有趣文章、工具和资源分享

---

 [← Back to Home](../)
